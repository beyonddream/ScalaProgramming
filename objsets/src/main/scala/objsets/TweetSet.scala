...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("empty set found")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val leftMostRetweeted: Tweet  = if (left.isInstanceOf[Empty]) elem else left mostRetweeted
    val rightMostRetweeted: Tweet = if (right.isInstanceOf[Empty]) elem else right mostRetweeted
