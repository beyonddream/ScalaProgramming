 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("max of a negative numbers") {
    assert(max(List(-1, -8, -120)) === -1)
  }

  test("max of a negative numbers with zero") {
    assert(max(List(-1, -2, -1, 0)) === 0)
  }

  test("max of a few numnbers along with Int.maxValue") {
    assert(max(List(1, Int.MaxValue, 12000, 33993, 12)) === Int.MaxValue)
  }
